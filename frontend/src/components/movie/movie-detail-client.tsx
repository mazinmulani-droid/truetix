...
export function MovieDetailClient({ id, initialMovie }: MovieDetailClientProps) {
  ...
  useEffect(() => {
    if (!initialMovie || !initialMovie.title) {
      const found = getMovieById(id);
      if (found) {
        setMovie(found);
      }
    }
  }, [id, initialMovie, _hasHydrated, getMovieById]);

  if (!movie) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center text-center p-4">
        <h1 className="text-2xl font-bold text-destructive mb-2">Film Not Found</h1>
        <p className="text-muted-foreground mb-6">The requested film could not be located in the catalog.</p>
        <Link href="/movies">
          <Button variant="outline">Browse All Films</Button>
        </Link>
      </div>
    );
  }
  ...
  showtimes.forEach((st: any) => {
    const dateStr = new Date(st.startTime).toLocaleDateString('en-GB', {
      weekday: 'short',
      day: '2-digit',
      month: 'short',
      year: 'numeric',
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="relative w-full h-[40vh] md:h-[60vh] bg-muted">
        <div
          ...
          style={{
            backgroundImage: `url(${movie.posterUrl || 'https://images.unsplash.com/photo-1536440136628-849c177e76a1?q=80&w=1200&auto=format&fit=crop'})`,
          }}
        />
        ...
        <div className="container mx-auto px-4 h-full relative z-10 flex flex-col justify-end pb-10">
          <div className="flex flex-col md:flex-row gap-8 items-end md:items-start">
            <div className="w-48 md:w-64 shrink-0 rounded-lg overflow-hidden shadow-2xl border-4 border-card transform translate-y-20 bg-card">
              <img
            ...
            <div className="flex-1 space-y-4 pt-10 md:pt-0">
              ...
              <div className="flex flex-wrap items-center gap-4 text-sm font-medium">
                ...
                <span className="flex items-center gap-1">
                  <Clock className="w-4 h-4" /> {movie.durationMinutes || 120} mins
                </span>
                <span className="flex items-center gap-1">
      ...
      <div className="container mx-auto px-4 py-24 md:py-16">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <section>
              <h3 className="text-2xl font-bold border-l-4 border-primary pl-4 mb-4">
                Director & Cast
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-card p-4 rounded-lg border border-border">
                  <p className="text-sm text-muted-foreground mb-1">Director</p>
                  <p className="font-bold">{movie.director || 'Jon Favreau'}</p>
                </div>
                <div className="bg-card p-4 rounded-lg border border-border">
                  <p className="text-sm text-muted-foreground mb-1">Cast</p>
                  <p className="font-bold">{movie.cast || 'Cast members'}</p>
                </div>
              </div>
            </section>
          ...
          <div id="showtimes-section" className="lg:col-span-1 space-y-6">
            <h3 className="text-2xl font-bold border-l-4 border-primary pl-4 mb-4 uppercase">
              Showtimes
            </h3>

            {Object.keys(showtimesByDate).length > 0 ? (
              <div className="space-y-6">
                {Object.entries(showtimesByDate).map(([date, showtimesList]) => (
                  <div key={date} className="bg-card/50 border border-border rounded-lg p-4">
                    ...
                    {(() => {
                      ...
                      showtimesList.forEach((st: any) => {
                        const cName = st.cinema?.name || 'TrueTix Leicester Square';
                          ...
                          <div className="flex flex-wrap gap-2">
                            {stList.map((st: any) => (
                              <Link key={st.id} href={`/booking/seats?showtimeId=${st.id}`}>
                                <Button
                                  variant="outline"
                                  className="border-primary/50 transition-colors hover:bg-primary hover:text-white"
                                >
                                  {new Date(st.startTime).toLocaleTimeString('en-GB', {
                                    hour: '2-digit',
                                    minute: '2-digit',
                                    hour12: false,
                                  })}
              ...
              <div className="bg-card border border-border rounded-lg p-8 text-center">
                <p className="text-muted-foreground">
                  No showtimes currently available for this film.
                </p>
              </div>
            )}
          </div>
